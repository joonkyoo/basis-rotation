...
class CommunicationHandler(object):
    ...
    def __init__(self, master_addr, master_port, rank,
        ...
        logging.info(f"Finished initializing process group; backend: {backend}, rank: {rank}, "
    ...
    def setup_queues(self):
        ...
        logging.info(f"Send ranks: {self.send_ranks}")
        logging.info(f"Receive ranks: {self.receive_ranks}")
    ...
    def create_process_groups(self):
        ...
        logging.info("Setting up process groups for broadcasts...")
...
def _recv(tensor_name, src_rank, tensor_shape=None, dtype=torch.float32,
    ...
    if sub_process_group is not None:
        received_tensor_shape = torch.zeros(len(tensor_shape), dtype=torch.int)
