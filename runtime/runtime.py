...
class StageRuntime:
    def __init__(self, model, distributed_backend, bf16,
                 training_tensor_shapes, eval_tensor_shapes,
                 training_tensor_dtypes, inputs_module_destinations,
                 target_tensor_names, configuration_maps, master_addr, master_port,
        ...
        self.initialize(model, inputs_module_destinations, configuration_maps,
                        master_addr, master_port, rank, local_rank, num_ranks_in_server)
    ...
    def initialize(self, model, inputs_module_destinations,
                   configuration_maps, master_addr, master_port, rank,
        ...
        if self.num_ranks_in_stage > 1:
            ...
            logging.info(f"Replicating stage: ranks={self.num_ranks_in_stage}, module_size={module_size:.3f}")
    ...
    def set_batch_sampler(self, func, is_eval=False):
        self.batch_sampler = func
        self.batch_sampler_is_eval = is_eval
    ...
    def receive_tensors_forward(self):
        ...
        self.tensors.append({})
        input = None
        if hasattr(self, 'loader_iter') and self.loader_iter is not None:
            input = next(self.loader_iter)
        elif hasattr(self, 'batch_sampler') and self.batch_sampler is not None:
            input = self.batch_sampler(eval=self.batch_sampler_is_eval)
    ...
    def _run_forward(self, tensors):
        ...
        with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.bf16):
            for i, (module, input_names, output_names) in \
                    enumerate(zip(modules, all_input_names, all_output_names)):
                if i == (len(modules) - 1) and self.is_criterion:
                    sz = tensors["target"].numel()
                    module_outputs = [module(tensors[input_name].view(sz, -1),
                                            tensors["target"].view(sz))
    ...
    def run_backward(self):
        ...
        # assumes tensors have the correct inputs from previous stage (done in receive_tensors_forward)
